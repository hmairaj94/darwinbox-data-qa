...
- Heuristic handling for messy Excel sheets: title/header offsets, multiple table regions separated by blank rows or columns, and flattened multi-row headers
- Detected source ranges, confidence scores, and warnings shown after upload so inferred tables can be verified
...
All non-secret application configuration lives in [`config.json`](config.json): model, upload limits, Excel detection thresholds, row limits, agent step/retry bounds, and join-hint threshold. Secrets stay in environment variables. Set `CONFIG_PATH` to use a different JSON file.
...
The main “delta” beyond an LLM wrapper is the deterministic data layer: Excel-region and header detection, normalized identifiers, profiling, cross-file join hints, AST-based SQL validation, table allow-listing, hard limits, and rule-based chart validation.
...
Tests cover Excel table/header detection, identifier normalization, join inference, chart selection, agent routing, tool-output processing, and rejection of unsafe SQL. A live Groq call is deliberately excluded from the unit suite.

## Known prototype limits

- Sessions are local files and have no authentication or automatic expiry.
- Excel detection is heuristic. Low-confidence ranges should be verified in the upload summary; embedded images, charts, pivot/dashboard layouts, and tables without blank separators are not interpreted.
